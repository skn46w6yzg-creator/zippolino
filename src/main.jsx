...
function App() {
  ...
  return <>
    ...
    <main id="top">
      ...
      <section id="cold-drinks" className="drink-category cold-section"><div className="section-head"><div><p className="eyebrow">COLD DRINKS</p><h2>Keep it cool.</h2></div><p>Choose your quantity and add any cold drink directly.</p></div><div className="cold-list">{MENU.coldDrinks.map(item => <div className="cold-row" key={item.id}><span>{item.name}</span><b>{money(item.price)}</b><div className="cold-actions"><Quantity value={coldQuantities[item.id] || 1} setValue={qty => setColdQuantities(current => ({ ...current, [item.id]: qty }))}/><button onClick={() => addColdDrink(item)} aria-label={`Add ${coldQuantities[item.id] || 1} ${item.name}`}>Add +</button></div></div>)}</div></section>
    ...
    <footer><a className="brand" href="#top"><Brand/></a><div><a href="#pancakes">Menu</a><a href="#story">About</a></div><p>© {new Date().getFullYear()} ZIPPOLINO · ONLINE ORDERS ONLY · {BUSINESS.address}</p></footer>
